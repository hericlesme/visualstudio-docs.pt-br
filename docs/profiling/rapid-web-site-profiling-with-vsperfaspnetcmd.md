...
## <a name="profiling-an-aspnet-application"></a>Criar um perfil de aplicativo ASP.NET
...
> [!NOTE]
> Por padrão, o prompt de comando não retorna após um comando **vsperfaspnetcmd**. Você pode usar a opção **/nowait** para forçar o prompt de comando a retornar. Confira [Usar a opção /NoWait](#UsingNoWait).
...
Caso deseje analisar arquivos .*dll* compilados estatisticamente no aplicativo Web, instrumente os arquivos usando a ferramenta de linha de comando [VSInstr](../profiling/vsinstr.md). O comando vsperfaspnetcmd /trace incluirá dados dos arquivos instrumentados.
