...
---
# <a name="control-where-r-code-runs-with-workspaces"></a>Controlar onde o código R é executado com espaços de trabalho
...
Para abrir a janela **Espaços de trabalho**, use o comando **Ferramentas do R** > **Windows** > **Espaços de trabalho** ou pressione **Ctrl**+**9**.
...
## <a name="save-and-reset-a-workspace"></a>Salvar e redefinir um espaço de trabalho
...
Os espaços de trabalho remotos permitem que você se conecte a uma sessão do R em um computador remoto. (Confira [Configurar espaços de trabalho remotos](setting-up-remote-r-workspaces.md) para saber como configurar um computador para essa finalidade.)
...
> [!Tip]
> Para desabilitar prompt de salvamento, selecione o comando **Ferramentas do R** > **Opções** e defina a opção **Mostrar caixa de diálogo de confirmação antes de alternar entre espaços de trabalho** como `No`. Consulte [Opções de espaço de trabalho](options-for-r-tools-in-visual-studio.md#workspace).
...
### <a name="switch-to-a-remote-workspace"></a>Alternar para um espaço de trabalho remoto
...
No entanto, esse aviso também será exibido se um *certificado autoassinado* tiver sido usado para habilitar o HTTPS no computador remoto em vez de usar outro de um provedor confiável. Para obter mais informações, confira [Configurar espaços de trabalho remotos](setting-up-remote-r-workspaces.md).
...
## <a name="copy-project-files-to-remote-workspaces"></a>Copiar arquivos de projeto para espaços de trabalho remotos
...
- Quando as RTVS estiverem associadas a um espaço de trabalho remoto e você pressionar **F5**, selecionar **Depurar** > **Iniciar Depuração** ou iniciar a execução do código de outro modo, as RTVS, por padrão, copiarão o arquivo do projeto para o espaço de trabalho remoto automaticamente (veja abaixo como controlar esse comportamento).
...
## <a name="copy-files-from-a-remote-workspace"></a>Copiar arquivos de um espaço de trabalho remoto
