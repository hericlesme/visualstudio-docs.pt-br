...
O item de menu **Ferramentas do R > Configurações da Ciência de Dados** também configura o IDE do Visual Studio com um layout que é otimizado para as necessidades dos cientistas de dados. Especificamente, essa opção abre as janelas [Interativo](interactive-repl.md), [Gerenciador de Variáveis](variable-explorer.md) e [Espaços de trabalho](workspaces.md):
