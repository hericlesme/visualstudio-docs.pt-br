...
- Visual Studio 2017 com a carga de trabalho do Python instalada. Para ver instruções, confira [Como instalar o suporte do Python no Visual Studio](installing-python-support-in-visual-studio.md).
...
1. Nos campos, na parte inferior da caixa de diálogo, insira as informações a seguir (conforme mostrado no gráfico anterior) e, em seguida, escolha **OK**:
    ...
    - **Criar um novo repositório Git**: escolha essa opção (que é clara por padrão) para que o Visual Studio crie um repositório Git local quando criar a solução.
...
1. Na barra de status do Visual Studio, marque o botão de confirmação não enviada (a seta para cima com um "2") para abrir a página **Sincronização** no **Team Explorer**. Como você tem apenas um repositório local, a página oferece opções simples para publicar o repositório em diferentes repositórios remotos.
    ...
    Você pode escolher o serviço que desejar para seus próprios projetos. Este tutorial mostra o uso do GitHub, onde o código de exemplo concluído do tutorial é mantido no repositório [Microsoft/python-sample-vs-learn-django](https://github.com/Microsoft/python-sample-vs-learn-django).
...
Agora que você configurou o controle do código-fonte para o projeto, você pode criar no ambiente virtual os pacotes necessários do Django que o projeto exige. Você pode usar o **Team Explorer** para excluir a pasta do ambiente do controle do código-fonte.
