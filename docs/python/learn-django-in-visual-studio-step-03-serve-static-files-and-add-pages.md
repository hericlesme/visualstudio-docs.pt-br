...
À medida que desenvolve um aplicativo do Django, geralmente você adiciona vários outros arquivos Python, HTML, CSS e JavaScript. Para cada tipo de arquivo (bem como outros arquivos, como `web.config`, de que você pode precisar para implantação), o Visual Studio fornece [modelos de item](python-item-templates.md) convenientes para você começar.
...
Você pode organizar os arquivos usando qualquer estrutura de pastas dentro de `static` e, em seguida, usar caminhos relativos dentro dessa pasta para fazer referência aos arquivos. Para demonstrar esse processo, as etapas a seguir adicionam um arquivo CSS ao aplicativo, depois usam essa folha de estilos no modelo de `index.html`:

1. No **Gerenciador de Soluções**, clique na pasta "HelloDjangoApp" no projeto do Visual Studio, selecione **Adicionar** > **Nova pasta** e o nome da pasta `static`.
...
1. Substitua o conteúdo do arquivo `templates/HelloDjangoApp/index.html` do aplicativo pelo código a seguir, que substitui o elemento `<strong>` usado na etapa 2 por um `<span>` que faz referência à classe de estilo `message`. O uso de uma classe de estilo dessa maneira oferece muito mais flexibilidade ao estilo do elemento. (Se você não se moveu `index.html` para uma subpasta em `templates`, veja [modelo namespacing](learn-django-in-visual-studio-step-02-create-an-app.md#template-namespacing) na etapa 2).
...
### <a name="question-what-is-purpose-of-the--load-staticfiles--tag"></a>Pergunta: Qual é a finalidade da marcação {% load staticfiles %}?
...
1. No **Gerenciador de Soluções**, clique com o botão direito na pasta `templates/HelloDjangoApp`, selecione **Adicionar** > **Novo item**, selecione o modelo de item "Página HTML", nomeie o arquivo `about.html` e selecione **OK**.
...
1. Abra o arquivo `urls.py` do projeto do Django e adicione a seguinte linha na matriz `urlPatterns`:

    ```python
    url(r'^about$', HelloDjangoApp.views.about, name='about'),
    ```

1. Abra o arquivo `templates/HelloDjangoApp/index.html` e adicione a seguinte linha abaixo do elemento `<body>` para criar um link para a página Sobre (novamente, substitua esse link por uma barra de navegação na etapa 3 – 4):
...
Resposta: Embora a função de exibição em `views.py` tenha o nome `index`, os padrões de roteamento de URL no arquivo `urls.py` do projeto Django não contêm uma expressão regular que corresponda à cadeia de caracteres "index". Para fazer a correspondência com essa cadeia de caracteres, você precisa adicionar outra entrada para o padrão `^index$`.
...
1. Modifique `templates/HelloDjangoApp/index.html` para se referir ao modelo de base e substitua o bloco de conteúdo. Veja que, usando a herança, esse modelo se torna simples:
...
1. Modifique `templates/HelloDjangoApp/about.html` para também se referir ao modelo de base e substitua o bloco de conteúdo:
