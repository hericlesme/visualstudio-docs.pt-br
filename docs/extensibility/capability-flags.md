...
|`SCC_CAP_IGNORECASE`|0x00800000L|Dá suporte à comparação de arquivo maiusculas de minúsculas.|  
|`SCC_CAP_IGNORESPACE`|0x01000000L|Oferece suporte ao arquivo de comparação que ignora o espaço em branco.|  
|`SCC_CAP_POPULATELIST`|0x02000000L|Dá suporte à localização de arquivos extras.|  
...
|`SCC_CAP_GET_NOUI`|0x20000000L|Plug-in não suporta uma interface de usuário para Get, mas ainda pode chamar a IDE [SccGet](../extensibility/sccget-function.md).|  
...
## <a name="capability-bits-added-in-version-13"></a>Bits de recurso adicionados na versão 1.3  
...
|------------------------------|-----------|-----------------|  
|`SCC_EXCAP_CHECKOUT_LOCALVER`|1|Oferece suporte a `SCC_CHECKOUT_LOCALVER` opção para check-outs.|  
...
|`SCC_EXCAP_POPULATELIST_DIR`|4|Oferece suporte a localizar os diretórios extras.|  
|`SCC_EXCAP_QUERYCHANGES`|5|Oferece suporte ao enumerar as alterações de arquivo.|  
...
|`SCC_EXCAP_THREADSAFE_QUERY_INFO`|8|Dá suporte à chamada SccQueryInfo em vários threads.|  
|`SCC_EXCAP_REMOVE_DIR`|9|Oferece suporte a função SccRemoveDir.|  
