...
> [!NOTE]
> Remoto depurar ASP.NET Core em vez disso, consulte [remota de depuração ASP.NET Core em um computador com IIS](../debugger/remote-debugging-aspnet-on-a-remote-iis-computer.md). Para o serviço de aplicativo do Azure, você pode facilmente implantar e depurar em uma instância pré-configurado do IIS usando o [instantâneo depurador](../debugger/debug-live-azure-applications.md) (.NET 4.6.1 necessário) ou por [anexar o depurador do Gerenciador de servidores](../debugger/remote-debugging-azure.md).
...
Depois que o aplicativo implanta com êxito, ele deve ser iniciado automaticamente. Se o aplicativo não for iniciado no Visual Studio, inicie o aplicativo no IIS.
