...
> [!WARNING]
>  As configurações de depuração do projeto no **propriedades de configuração/depuração** categoria para aplicativos UWP e componentes que são escritos em C++ são diferentes. Consulte [iniciar uma sessão de depuração (VB, c#, C++ e XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md).  
  
 Especifica qual depurador a ser usado no **depurador a iniciar** caixa de listagem. Sua escolha afetará as propriedades visíveis.  
...
|-----------------|---------------------|  
|**Depurador a iniciar**|Especifica o depurador para executar, com as seguintes opções:<br /><br /> -   **Depurador local do Windows**<br />-   **Depurador remoto do Windows**<br />-   **Depurador do navegador da Web**<br />-   **Depurador de serviço da Web**|  
...
|**Anexar** (Depurador Local do Windows e o depurador remoto do Windows)|Especifica se inicia ou anexa ao aplicativo. A configuração padrão é Não.|  
|**Nome do servidor remoto** (depurador remoto do Windows)|Especifica o nome de um computador (diferente do seu) no qual você deseja depurar um aplicativo.<br /><br /> A macro RemoteMachine compilação é definida como o valor dessa propriedade; Para obter mais informações, consulte [Macros para compilar comandos e propriedades](/cpp/ide/common-macros-for-build-commands-and-properties).|  
|**Conexão** (depurador remoto do Windows)|Permite que você alterne entre tipos de conexão padrão e sem autenticação para depuração remota. Especifique um nome de computador remoto no **nome do servidor remoto** caixa. Tipos de conexão incluem o seguinte:<br /><br /> -   **Remoto com autenticação do Windows**<br />-   **Remoto sem autenticação**<br /><br /> **Observação** depuração remota com autenticação não pode deixar o computador remoto vulnerável a violações de segurança. O modo de Autenticação do Windows é mais seguro.<br /><br /> Para obter mais informações, consulte [instalação de depuração remota](../debugger/remote-debugging.md).|  
|**URL de HTTP** (Web Service depurador e o depurador do navegador da Web)|Especifica a URL no qual o projeto que você está depurar está localizado.|  
|**Tipo de Depurador**|Especifica o tipo de depurador a ser usado: **apenas nativo**, **apenas gerenciado**, **somente GPU**, **Mixed**, **automática**(padrão), ou **Script**.<br /><br /> -   **Somente nativo** é para código C++ não gerenciado.<br />-   **Somente gerenciado** é para o código que é executado sob o common language runtime (código gerenciado).<br />-   **Misto** invoca depuradores para gerenciado e código não gerenciado.<br />-   **Auto** determina o tipo de depurador com base no compilador e informações de EXE.<br />-   **Script** invoca um depurador de scripts.<br />-   **Somente GPU** é para código C++ AMP que é executado em um dispositivo GPU ou sobre o rasterizador de referência do DirectX. Consulte [depurando código de GPU](../debugger/debugging-gpu-code.md).|  
|**Ambiente** (Depurador Local do Windows e o depurador remoto do Windows)|Especifica variáveis de ambiente para o programa que você está depurando. Use a sintaxe de variável de ambiente padrão (por exemplo, `PATH="%SystemRoot%\..."`). Essas variáveis substituir o ambiente do sistema ou são mescladas com o ambiente do sistema, dependendo do **mesclar ambiente** configuração. Quando você clica na coluna de configurações, um "Editar..." aparece. Clique nesse link para editar variáveis de ambiente.|  
|**Mesclar ambiente** (Depurador Local do Windows)|Determina se as variáveis que são especificados no **ambiente** caixa será mesclada com o ambiente que é definido pelo sistema operacional. A configuração padrão é Sim.|  
|**A depuração SQL** (todos, exceto o depurador de Cluster MPI)|Permite a depuração de procedimentos SQL do seu aplicativo [!INCLUDE[vcprvc](../code-quality/includes/vcprvc_md.md)]. A configuração padrão é Não.|  
|**Tipo de acelerador de depuração** (depuração GPU apenas)|Especifica o dispositivo GPU a ser usado para depuração. A instalação de drivers de dispositivo para dispositivos compatíveis com GPU adicionará outras opções. A configuração padrão é “GPU - emulador de software.”|  
|**Comportamento de ponto de interrupção padrão da GPU** (depuração GPU apenas)|Especifica se um evento de ponto de interrupção deve ser gerado para cada thread em um warp SIMD. A configuração padrão é gerar o evento do ponto de interrupção apenas uma vez por encurvamento.|  
|**Amp padrão Accelerator**|Especifica o acelerador padrão de AMP ao depurar o código de GPU. Escolha **acelerador de software WARP** para investigar se um problema é causado por hardware ou um driver em vez de seu código.|  
|**Diretório de implantação** (depurador remoto do Windows)|Especifica o caminho no computador remoto onde a saída do projeto será copiada antes da inicialização. O caminho pode ser um compartilhamento de rede no computador remoto, ou pode ser um caminho para uma pasta no computador remoto. A configuração padrão está vazia, o que significa que a saída do projeto não é copiada para um compartilhamento de rede. Para habilitar a implantação dos arquivos, você também deve selecionar o **implantar** caixa de seleção na caixa de diálogo Gerenciador de configuração. Para obter mais informações, consulte [Como criar e editar configurações](../ide/how-to-create-and-edit-configurations.md).|  
...
|**Gerar arquivo de mapa** ([/MAPEAR](/cpp/build/reference/map-generate-mapfile))|Informe ao vinculador para gerar um arquivo de mapa durante a vinculação. A configuração padrão é Não. Para obter mais informações, consulte [/MAP (Gerar Mapfile)](/cpp/build/reference/map-generate-mapfile).|  
|**Mapear o nome de arquivo** ([/MAPEAR:](/cpp/build/reference/map-generate-mapfile)*nome*)|Se você escolher Gerar Arquivo de Mapa, poderá especificar o arquivo de mapa nesta caixa. Para obter mais informações, consulte [/MAP (Gerar Mapfile)](/cpp/build/reference/map-generate-mapfile).|  
...
Para depurar os tipos de projeto, como bibliotecas estáticas e DLLs, seu projeto do Visual Studio deve ser capaz de encontrar os arquivos corretos. Quando o código-fonte estiver disponível, você pode adicionar bibliotecas estáticas e DLLs como projetos separados para a mesma solução (torna isso fácil de depuração). Para obter informações sobre como criar esses tipos de projeto, consulte [criando e usando uma biblioteca de vínculo dinâmico (DLL)](/cpp/build/walkthrough-creating-and-using-a-dynamic-link-library-cpp) e [criar um usando uma biblioteca estática](/cpp/windows/walkthrough-creating-and-using-a-static-library-cpp). Com o código-fonte disponível, você também pode criar um novo projeto do Visual Studio escolhendo **arquivo > Novo > projeto de código existente**.

Para depurar DLLs que são externos ao seu projeto, consulte [projetos DLL depuração](../debugger/debugging-dll-projects.md#vxtskdebuggingdllprojectsexternal). Se você precisar depurar seu próprio projeto DLL, mas não tem acesso ao projeto para o aplicativo de chamada, consulte [como depurar um projeto de DLL](../debugger/how-to-debug-from-a-dll-project.md).
