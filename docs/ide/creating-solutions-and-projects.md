...
*Projetos* são contêineres lógicos no Visual Studio que contêm os itens necessários para criar seu aplicativo, como arquivos de código-fonte, bitmaps, ícones e referências de componente e serviço. Quando você cria um novo projeto, o Visual Studio cria um *solução* para conter os projetos. Então, é possível adicionar projetos novos ou existentes à solução se você quiser. As soluções também podem conter arquivos não conectados a nenhum projeto específico.   
...
1. Há várias maneiras de criar um novo projeto no Visual Studio. Na Página Inicial, digite o nome de um modelo de projeto na caixa **Pesquisar modelos de projeto** ou escolha o link **Criar novo projeto** para abrir a caixa de diálogo **Novo Projeto**. Você também pode escolher **Arquivo**, **Novo**, **Projeto...** no menu ou escolher o botão **Novo Projeto** na barra de ferramentas.
...
1. Para mostrar a lista de modelos da linguagem de programação que você deseja usar, escolha o triângulo próximo do nome da linguagem e, em seguida, escolha um tipo de projeto. O exemplo a seguir mostra os modelos de projeto disponíveis para projetos Web do Visual C#.

  ![Modelos de projeto](./media/vside-newproject-projects-list.png)
...
1. Se você quiser adicionar mais um projeto a uma solução, escolha o nó da solução no Gerenciador de Soluções e, em seguida, no menu, escolha **Projeto**, **Adicionar novo item**.  
...
> [!NOTE]
>  Essa opção funciona melhor para coleções de arquivos relativamente simples. Atualmente, somente os tipos de projeto do Visual C++, Apache Cordova, Visual Basic e Visual C# são compatíveis.  
...
Ao criar um projeto, você pode determinar uma versão específica do .NET Framework que você deseja que o projeto use. Para especificar uma versão do .NET Framework, escolha o menu suspenso de versão do **.NET Framework** na caixa de diálogo **Novo Projeto**. Se estiver criando um projeto de um modelo do .NET Core, a versão do .NET Framework que você selecionar na lista suspensa será ignorada.  

![Seletor de versão do .NET Framework](./media/vside-newproject-framework.png)

> [!TIP]
>  Se você definir a versão do .NET Framework antes de selecionar um modelo de projeto, o Visual Studio mostrará apenas os modelos que são compatíveis com essa versão do .NET Framework.
...
Como mencionado anteriormente, você também pode abrir projetos de código sem precisar de uma solução. Para obter informações sobre como abrir o código sem a necessidade de uma solução, consulte [Desenvolver código no Visual Studio sem projetos ou soluções](../ide/develop-code-in-visual-studio-without-projects-or-solutions.md).
...
3.  Escolha a pasta que contém a solução e, em seguida, escolha a tecla DELETE.
