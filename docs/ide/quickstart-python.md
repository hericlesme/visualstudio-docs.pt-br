...
1. Na caixa de diálogo **Novo projeto**, no painel esquerdo, expanda **Outras linguagens** e selecione **Python**. No painel central, escolha **Projeto Web**, nomeie o projeto como “HelloPython” e selecione **OK**.
...
Aplicativos Web em Python quase sempre usam uma das muitas bibliotecas Python disponíveis para lidar com detalhes de baixo nível como o roteamento de solicitações da Web e formatação de respostas. A carga de trabalho de desenvolvimento de Python no Visual Studio fornece [uma variedade de modelos para aplicativos Web](../python/template-web.md) compilados em torno de bibliotecas de Bottle, Flask e Django.

Neste guia de início rápido, no entanto, você usará uma biblioteca diferente, [Falcon](https://falconframework.org/), para experimentar o processo de instalação de um pacote e criar o aplicativo Web do zero. Para simplificar, as etapas a seguir instalam o Falcon no ambiente global padrão.
...
1. Clique com o botão direito do mouse no projeto no **Gerenciador de Soluções** e selecione **Adicionar > Novo Item...**. Na caixa de diálogo exibida, selecione **Arquivo Python vazio**, nomeie-o como `hello.py` e escolha **OK**. O Visual Studio abre o arquivo automaticamente em uma janela do editor. (Em geral, o comando **Adicionar > Novo item...** é uma ótima maneira de adicionar tipos diferentes de arquivos a um projeto, pois os modelos de item geralmente fornecem código clichê útil.)

1. Copie e cole, ou digite o código a seguir em `hello.py`:
...
1. Aparece uma janela de comando com a mensagem “Iniciando servidor de aplicativo Web", em seguida, uma janela do navegador é aberta para `localhost:8080`, em que é possível ver a mensagem “Olá, Python!” A solicitação GET também aparece na janela de comando. (Se você vir somente o shell interativo de Python na janela Comando ou se essa janela piscar brevemente na tela, verifique se você definiu `hello.py` como o arquivo de inicialização na etapa 1 acima.)

1. Feche a janela de comando para interromper o aplicativo.
...
> [Tutorial: introdução ao Python no Visual Studio](../python/vs-tutorial-01-01.md).

- Saiba mais sobre os [Modelos de aplicativo Web Python no Visual Studio](../python/template-web.md)
- Saiba mais sobre o [IDE do Visual Studio](../ide/visual-studio-ide.md)
- Aprenda a usar o [Depurador do Visual Studio](../debugger/debugger-feature-tour.md)