...
```  
 ...
 Se você comentar a linha de código que usa `bind`, descomentar a linha de código que chama `addEventListener` sem `bind` e executar o código novamente, a função `dataReadyHandler` falhará. Por exemplo, em `dataReadyHandler`, `this.name` será indefinido e `this.data()` resultará em um erro porque o objeto `this` não se refere mais ao objeto de dados.  
