---
title: "Função ATANH (JavaScript) | Microsoft Docs"
ms.custom: 
ms.date: 01/18/2017
ms.prod: windows-client-threshold
ms.reviewer: 
ms.suite: 
ms.technology: devlang-javascript
ms.tgt_pltfrm: 
ms.topic: language-reference
dev_langs:
- JavaScript
- TypeScript
- DHTML
ms.assetid: effa2a59-9f2a-4586-8a02-bc8face582c7
caps.latest.revision: "3"
author: mikejo5000
ms.author: mikejo
manager: ghogen
ms.openlocfilehash: 3b1afab88630168573771fcae4710c623ce8d9ba
ms.sourcegitcommit: aadb9588877418b8b55a5612c1d3842d4520ca4c
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/27/2017
---
# <a name="mathatanh-function-javascript"></a>Função Math.atanh (JavaScript)
Retorna a tangente hiperbólica inversa de um número.  
  
## <a name="syntax"></a>Sintaxe  
  
```  
  
Math.atanh(x)   
```  
  
## <a name="remarks"></a>Comentários  
 Se `x` for `NaN`, o resultado é `NaN`.  
  
 **Aplica-se a**: [objeto Math](../../javascript/reference/math-object-javascript.md)  
  
## <a name="requirements"></a>Requisitos  
 [!INCLUDE[jsv12](../../javascript/reference/includes/jsv12-md.md)]  
  
## <a name="see-also"></a>Consulte também  
 [Função ACOS](../../javascript/reference/math-acos-function-javascript.md)   
 [Função ACOSH](../../javascript/reference/math-acosh-function-javascript.md)   
 [Função ASIN](../../javascript/reference/math-asin-function-javascript.md)   
 [Função Math. asinh](../../javascript/reference/math-asinh-function-javascript.md)   
 [Função ATAN](../../javascript/reference/math-atan-function-javascript.md)   
 [Função Math. ATAN2](../../javascript/reference/math-atan2-function-javascript.md)   
 [Função Math.CoS](../../javascript/reference/math-cos-function-javascript.md)   
 [Função COSH](../../javascript/reference/math-cosh-function-javascript.md)   
 [Função Sin](../../javascript/reference/math-sin-function-javascript.md)   
 [Função SINH](../../javascript/reference/math-sinh-function-javascript.md)   
 [Função Math.tanh](../../javascript/reference/math-tanh-function-javascript.md)